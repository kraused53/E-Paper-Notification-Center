...
{2}% cloud cover'''.format(cur_temp, cur_hum, cur_clouds)
        ...
        # Place city name
        draw.text((x, y), city_name, display.RED, font)
        
        # Make text bigger for time
        ...
        y = 50 - (h/2)
        # Place Time
        draw.text((x, y), cur_time, (255, 0, 0), font)
        ...
        draw.text((150, 150), data_block, display.BLACK, font)
        font = ImageFont.truetype("LiberationMono-Regular.ttf", 16)
                ...
                if weather_data.time is not None:
                        if weather_data.sunset is not None:
                                if weather_data.sunrise is not None:
                                        ...
                                        if icon is not None:
                                                # Paste Candle Icon
                                                Image.Image.paste(img, icon, (display.WIDTH-50, display.HEIGHT-50))
        ...
        draw.line([(15, 100), (385, 100)], fill=(0, 0, 0))
        # Draw line to isolate icon
        draw.line([(100, 115), (100, 285)], fill=(0, 0, 0))
